...
const AddCategory = () => {
  ...
  const addCategory = async (e) => {
    ...
    if (!formFields.name || !formFields.color || !file || file.length === 0) {
      context.setAlertBox({
        open: true,
        error: true,
    ...
    try {
      setIsLoading(true);
      await postData("/api/category", formData); // backend handles upload
      await context.loadCategories(); // refresh category list
      navigate("/category");
    } catch (err) {
      console.error(err);
      context.setAlertBox({
      ...
      <div className="card shadow border-0 w-100 p-4">
        ...
        <div className="ml-auto d-flex align-items-center presentation breadcrumbs">
          <Breadcrumbs aria-label="breadcrumb">
            ...
            />
            <StyledBreadcrumb component="a" href="/category" label="Category" />
          </Breadcrumbs>
        </div>
      </div>
